...
def get_firestore_client():
    ...
    try:
        ...
        if "firestore_credentials" in st.secrets:
            ...
            st.sidebar.warning(
                "Tidak ada 'firestore_credentials' di st.secrets. Mencoba koneksi default Firestore.")  # Translated
        return db
        ...
        try:
            ...
            for index, row in df_sku_master.iterrows():
                ...
                if code and jenis_key and jenis_key in sku_decoder:
                    ...
                    st.warning(
                        f"Jenis '{jenis_raw_from_excel}' untuk kode '{code}' tidak dikenali di Master SKU. Data ini mungkin tidak digunakan.")  # Translated
            return sku_decoder
        except Exception as e_load_sku:
            st.error(
                f"Gagal memuat Data Master SKU. Pastikan format file benar dan memiliki kolom 'CODE', 'ARTI', 'JENIS'. Error: {e_load_sku}")  # Translated
            return {}
    return {}
        ...
        try:
            ...
            if file_type == "sales":
                ...
                if 'No Transaksi' not in df.columns:
                    st.warning(
                        "Kolom 'No Transaksi' (atau variasi seperti 'No. Transaksi', 'ID Transaksi') tidak ditemukan di data penjualan. Jumlah pesanan akan dihitung berdasarkan baris unik.")
                    df['No Transaksi'] = df.index.astype(
                        str)  # Use row index as a dummy transaction ID, convert to string
                ...
                if 'Tanggal' not in df.columns:
                    raise KeyError(
                        "Kolom 'Tanggal' tidak ditemukan setelah pembersihan dan penamaan ulang di Data Inbound.")  # Translated
                df['Tanggal'] = pd.to_datetime(df['Tanggal'], errors='coerce')

                # Apply clean_financial_string function to financial columns
                for column_name in ['Qty Dipesan Unit', 'Qty Diterima', 'Harga', 'Amount', 'Sub Total', 'Diskon',
                                    'Pajak Total', 'Grand Total']:
                    df[column_name] = df[column_name].apply(clean_financial_string)
...
def parse_sku(sku, sku_decoder):
    ...
    if len(sku_upper) >= 3:
        ...
        sku_info["Category"] = sku_decoder.get("CATEGORY", {}).get(category_code_extracted,
                                                                   "Unknown Category")  # Translated

    # 3. Sub Category: Take the first 4 letters and numbers of the SKU product
    if len(sku_upper) >= 4:
        sub_category_code_extracted = sku_upper[:4]
        sku_info["Sub Category"] = sku_decoder.get("SUB_CATEGORY", {}).get(sub_category_code_extracted,
                                                                           "Unknown Sub Category")  # Translated
    ...
    if match:
        ...
        if year_or_deffect_code.startswith('D'):
            ...
            sku_info["Tahun Produksi"] = sku_decoder.get("TAHUN PRODUKSI", {}).get(year_or_deffect_code,
                                                                                   "Unknown Tahun")  # Translated

        # Continue with other lookups
        sku_info["Season"] = sku_decoder.get("SEASON", {}).get(season_code, "Unknown Musim")  # Translated
        sku_info["Singkatan Nama Produk"] = sku_decoder.get("SINGKATAN_NAMA_PRODUK", {}).get(product_name_code,
                                                                                             "Unknown Produk")  # Translated
    ...
    try:
        ...
        for key, df in dataframes.items():
            if not df.empty:
                ...
                data_to_save = df.applymap(lambda x: x.isoformat() if isinstance(x, datetime) else x).to_dict(
                    orient='records')
        ...
        for key in loaded_dataframes.keys():
            ...
            if doc.exists and "data" in doc.to_dict():
                ...
                if key == 'df_sales_combined' and 'No Transaksi' not in df.columns:
                    st.warning(
                        f"Menambahkan kolom 'No Transaksi' ke {key} saat memuat dari admin karena tidak ditemukan.")
    ...
    if st.session_state.get('is_admin', False):  # Only display uploader if admin
        ...
        if uploaded_sales_file:
            if temp_sku_decoder:  # Ensure SKU decoder exists
                with st.spinner("Memproses Data Penjualan..."):  # Translated
                    ...
                    if not df_sales_raw.empty:
                        if 'SKU' in df_sales_raw.columns:
                            ...
                            st.sidebar.warning(
                                "Kolom 'SKU' tidak ditemukan di Data Penjualan. Parsing SKU dilewati.")  # Translated
                            temp_df_sales = df_sales_raw

                        # --- ADDED ROBUSTNESS CHECK FOR 'No Transaksi' HERE ---
                        if 'No Transaksi' not in temp_df_sales.columns:
                            st.warning(
                                "Menambahkan kolom 'No Transaksi' ke data penjualan karena tidak ditemukan setelah pemrosesan.")
                ...
                st.sidebar.warning(
                    "Unggah Data Master SKU terlebih dahulu untuk parsing SKU pada Data Penjualan.")  # Translated
        ...
        if uploaded_inbound_file:
            if temp_sku_decoder:
                with st.spinner("Memproses Data Inbound..."):  # Translated
                    ...
                    if not df_inbound_raw.empty:
                        if 'SKU' in df_inbound_raw.columns:
                            ...
                            st.sidebar.warning(
                                "Kolom 'SKU' tidak ditemukan di Data Inbound. Parsing SKU dilewati.")  # Translated
                ...
                st.sidebar.warning(
                    "Unggah Data Master SKU terlebih dahulu untuk parsing SKU pada Data Inbound.")  # Translated
        ...
        if uploaded_stock_file:
            if temp_sku_decoder:
                with st.spinner("Memproses Data Stok..."):  # Translated
                    ...
                    if not df_stock_raw.empty:
                        if 'SKU' in df_stock_raw.columns:
                            ...
                            st.sidebar.warning(
                                "Kolom 'SKU' tidak ditemukan di Data Stok. Parsing SKU dilewati.")  # Translated
                ...
                st.sidebar.warning(
                    "Unggah Data Master SKU terlebih dahulu untuk parsing SKU pada Data Stok.")  # Translated
...
def plot_forecast_results(historical_data, forecast_values, prediction_type, model_name, forecast_horizon):
    ...
    st.dataframe(forecast_values.apply(
        lambda x: f"Rp {x:,.2f}" if prediction_type == "Penjualan Bersih" else f"{x:,.0f} unit"))  # Translated


# --- Main Dashboard ---
st.title("Dashboard Analisis Data Bisnis")  # Translated
st.markdown(
    "Dashboard ini membantu Anda menganalisis data penjualan, inbound, dan stok untuk mendapatkan wawasan bisnis.")  # Translated
...
if 'current_user_id' in st.session_state and st.session_state['current_user_id'] and \
    ...
    selected_categories = st.sidebar.multiselect("Filter Berdasarkan Kategori", all_categories,
                                                 default='Semua Kategori')  # Translated
    ...
    if 'Lokasi' in st.session_state['df_sales_combined'].columns:
        ...
        selected_locations = st.sidebar.multiselect("Filter Berdasarkan Lokasi Penjualan", all_locations,
                                                    default='Semua Lokasi')  # Translated
        ...
        st.sidebar.warning(
            "Kolom 'Lokasi' tidak ditemukan di Data Penjualan. Filter lokasi tidak tersedia.")  # Translated

    # NEW: Product Name Filter
    if 'Nama Barang' in st.session_state['df_sales_combined'].columns:
        all_product_names = ['Semua Produk'] + list(
            st.session_state['df_sales_combined']['Nama Barang'].unique())  # Translated
        selected_product_names = st.sidebar.multiselect("Filter Berdasarkan Nama Produk", all_product_names,
                                                        default='Semua Produk')  # Translated
        if 'Semua Produk' not in selected_product_names:  # Translated
            ...
            if 'Nama Barang' in df_inbound_filtered.columns:  # Assuming inbound also has 'Nama Barang' or similar
                df_inbound_filtered = df_inbound_filtered[
                    df_inbound_filtered['Nama Barang'].isin(selected_product_names)]
    else:
        st.sidebar.warning(
            "Kolom 'Nama Barang' tidak ditemukan di Data Penjualan. Filter nama produk tidak tersedia.")  # Translated
    ...
    with tab1:
        ...
        if not sales_by_category.empty:
            ...
            if selected_category_for_drilldown != 'Pilih Kategori':  # Translated
                st.subheader(
                    f"Penjualan Berdasarkan Sub Kategori dalam Kategori: {selected_category_for_drilldown}")  # Translated
                df_sales_drilldown = df_sales_filtered[df_sales_filtered['Category'] == selected_category_for_drilldown]
                sales_by_subcategory_drilldown = df_sales_drilldown.groupby('Sub Category')[
                    'Sub Total'].sum().sort_values(ascending=False).reset_index()

                if not sales_by_subcategory_drilldown.empty:
                    fig_sales_subcategory_drilldown = px.bar(sales_by_subcategory_drilldown, x='Sub Category',
                                                             y='Sub Total',
                                                             title=f'Penjualan per Sub Kategori di {selected_category_for_drilldown}',
                                                             # Translated
                                                             labels={'Sub Total': 'Total Penjualan (Rp)'},  # Translated
                                                             color='Sub Category',
                                                             template='plotly_white')
                    ...
                    st.info(
                        f"Tidak ada data sub kategori untuk kategori '{selected_category_for_drilldown}' dalam filter saat ini.")  # Translated
    ...
    with tab8:  # New tab for defect product analysis
        ...
        if not df_deffect_sales.empty:
            ...
            top_deffect_products_qty = df_deffect_sales.groupby('Nama Barang')['QTY'].sum().sort_values(
    ...
    with tab9:  # New tab for Sales Prediction
        ...
        if not df_sales_filtered.empty:
            ...
            forecast_horizon = st.slider("Horizon Prediksi (bulan ke depan)", min_value=1, max_value=6,
                                         value=3)  # Translated
            ...
            if model_choice == "Rata-rata Bergerak":  # Translated
                window_size = st.slider("Ukuran Jendela Rata-rata Bergerak (bulan)", min_value=1,
                                        max_value=len(data_to_predict) - 1 if len(data_to_predict) > 1 else 1,
                                        value=min(3, len(data_to_predict) - 1 if len(
                                            data_to_predict) > 1 else 1))  # Translated
                if window_size < 1:
                ...
                future_dates = pd.date_range(start=last_date + pd.DateOffset(months=1), periods=forecast_horizon,
                                             freq='MS')
                forecast_values = pd.Series([last_ma_value] * forecast_horizon, index=future_dates)
                plot_forecast_results(data_to_predict, forecast_values, prediction_type, "Rata-rata Bergerak",
                                      forecast_horizon)

            elif model_choice == "Exponential Smoothing (ETS)":  # Translated
                if len(data_to_predict) < 2:
                    st.warning(
                        "Tidak cukup data untuk model Exponential Smoothing. Diperlukan minimal 2 titik data.")  # Translated
                    st.stop()  # Changed return to st.stop()
                ...
                try:
                    ...
                    model = ExponentialSmoothing(data_to_predict, trend='add', seasonal=None,
                ...
                if len(data_to_predict) < (p_order + d_order + q_order + 1):
                    st.warning(
                        "Tidak cukup data untuk order ARIMA yang dipilih. Coba kurangi order atau berikan lebih banyak data.")  # Translated
                    st.stop()  # Changed return to st.stop()
                ...
                try:
                    ...
                    forecast_values = forecast
                    plot_forecast_results(data_to_predict, forecast_values, prediction_type,
                                          f"ARIMA {p_order},{d_order},{q_order}", forecast_horizon)

                except Exception as e_arima:
                    st.error(f"Gagal menjalankan model ARIMA. Error: {e_arima}")  # Translated
                    st.info(
                        "Coba sesuaikan order ARIMA (p, d, q) atau pastikan data Anda cukup stasioner.")  # Translated
                ...
                try:
                    ...
                    plot_forecast_results(data_to_predict, forecast_values, prediction_type, "Prophet",
                                          forecast_horizon)

                except Exception as e_prophet:
                    st.error(f"Gagal menjalankan model Prophet. Error: {e_prophet}")  # Translated
                    st.info(
                        "Pastikan data Anda memiliki minimal 2 titik data dan tidak ada nilai yang hilang.")  # Translated
        else:
            st.info(
                "Tidak ada data penjualan yang tersedia untuk prediksi dalam rentang filter yang dipilih.")  # Translated
        ...
        if not df_sales_filtered.empty:
            ...
            if comparison_type == "Tahun-ke-Tahun (Year-over-Year)":  # Translated
                ...
                comparison_data.index = pd.to_datetime(
                    comparison_data.index.map(lambda x: f"2000-{x}-01"))  # Dummy year for plotting
                ...
                if not comparison_data.empty:
                    ...
                    st.dataframe(comparison_data.applymap(lambda
                                                              x: f"Rp {x:,.2f}" if comparison_metric != "Jumlah Terjual (QTY)" else f"{x:,.0f} unit"))  # Translated
                else:
                ...
                monthly_data['Periode'] = pd.to_datetime(
                    monthly_data['Tahun'].astype(str) + '-' + monthly_data['Bulan'].astype(str))
                ...
                if not monthly_data.empty:
                    ...
                    monthly_data['MoM_Growth_Rate'] = (monthly_data['MoM_Change'] / monthly_data[
                    ...
                    st.dataframe(monthly_data[['Periode', metric_col, 'MoM_Change', 'MoM_Growth_Rate']].style.format({
                        metric_col: (lambda
                                         x: f"Rp {x:,.2f}" if comparison_metric != "Jumlah Terjual (QTY)" else f"{x:,.0f} unit"),
                        # Translated
                        'MoM_Change': (lambda
                                           x: f"Rp {x:,.2f}" if comparison_metric != "Jumlah Terjual (QTY)" else f"{x:,.0f} unit"),
                        # Translated
    ...
    def safe_qcut(series, q=5, ascending=True):
        ...
        if series.nunique() < q:
            ...
            if ascending:
                ...
                return ((ranked_series - 1) / (max_rank - 1) * (q - 1) + 1).astype(int) if max_rank > 1 else (
                    ranked_series).astype(int)
            else:
    ...
    with tab11:  # New tab for Customer Analysis with RFM
        ...
        if 'Channel' in df_sales_filtered.columns and 'Customer ID' in df_sales_filtered.columns:
            ...
            if selected_channel_for_customer_analysis != 'Semua Channel':  # Translated
                df_customer_analysis = df_customer_analysis[
                    df_customer_analysis['Channel'] == selected_channel_for_customer_analysis]  # Filter by 'Channel'
            ...
            if not df_customer_analysis.empty:
                ...
                with col_cust1:
                    display_kpi_card("Total Pelanggan Unik", f"{customer_summary['Customer ID'].nunique():,.0f}",
                                     "#FF5722")  # Translated
                with col_cust2:
                    display_kpi_card("Rata-rata Penjualan per Pelanggan",
                                     f"Rp {customer_summary['Total_Sales'].mean():,.2f}", "#607D8B")  # Translated
                with col_cust3:
                    display_kpi_card("Rata-rata Pesanan per Pelanggan",
                                     f"{customer_summary['Number_of_Orders'].mean():,.2f}", "#795548")  # Translated
                ...
                fig_customer_sales_dist = px.histogram(customer_summary, x='Total_Sales', nbins=20,
                                                       ...
                                                       labels={'Total_Sales': 'Total Penjualan (Rp)',
                                                               'count': 'Jumlah Pelanggan'},  # Translated
                                                       template='plotly_white')
                ...
                if not df_customer_analysis.empty and 'Tanggal' in df_customer_analysis.columns:
                    ...
                    current_date_for_rfm = df_customer_analysis['Tanggal'].max() + pd.Timedelta(
                        days=1)  # One day after the last transaction
                    ...
                    # Calculate Frequency
                    rfm_frequency = df_customer_analysis.groupby('Customer ID')['No Transaksi'].nunique().reset_index(
                        name='Frequency')

                    # Calculate Monetary
                    rfm_monetary = df_customer_analysis.groupby('Customer ID')['Nett Sales'].sum().reset_index(
                        name='Monetary')
                    ...
                    rfm_df['F_Score'] = safe_qcut(rfm_df['Frequency'], q=5,
                                                  ascending=True)  # Higher frequency is better
                    ...
                    rfm_df['RFM_Score'] = rfm_df['R_Score'].astype(str) + rfm_df['F_Score'].astype(str) + rfm_df[
                        'M_Score'].astype(str)
                ...
                st.info(
                    "Tidak ada data pelanggan yang tersedia untuk analisis dalam filter yang dipilih.")  # Translated
        else:
            st.warning(
                "Kolom 'Channel' atau 'Customer ID' tidak ditemukan di Data Penjualan. Analisis pelanggan tidak tersedia.")  # Translated
    ...
    if not df_stock_filtered.empty and not df_sales_filtered.empty:
        ...
        if not df_stock_top_20.empty:
            ...
            if not products_below_min_stock_top_20.empty:
                st.warning(
                    f"⚠️ **{len(products_below_min_stock_top_20)} dari 20 produk terlaris berada di bawah ambang batas stok minimum ({min_stock_threshold} unit)!**")  # Translated
                st.dataframe(products_below_min_stock_top_20[[
                    'Nama Item', 'SKU', 'Category', 'Lokasi', 'Tersedia', 'Dipesan'
                ]].sort_values(by='Tersedia'))
                st.info(
                    "Rekomendasi: Pertimbangkan untuk segera melakukan pemesanan ulang untuk produk-produk ini guna menghindari kehabisan stok dan potensi kehilangan penjualan.")  # Translated
            else:
                st.success(
                    "✅ Semua 20 produk terlaris berada di atas ambang batas stok minimum yang ditetapkan.")  # Translated
        else:
            st.info("Tidak ada data stok untuk 20 produk terlaris dalam rentang filter yang dipilih.")  # Translated
    else:
        st.info(
            "Tidak ada data penjualan atau stok yang tersedia untuk notifikasi stok minimum Top 20 Produk.")  # Translated
    ...
    if not low_stock_high_sales.empty:
        ...
        st.info(
            "Rekomendasi: Pertimbangkan untuk melakukan pemesanan ulang segera untuk produk-produk ini guna menghindari kehabisan stok dan potensi kehilangan penjualan.")  # Translated
    else:
    ...
    if not high_stock_low_sales.empty:
        ...
        st.info(
            "Rekomendasi: Pertimbangkan strategi promosi, diskon, atau penjualan cepat untuk produk-produk ini guna mengurangi biaya penyimpanan dan membebaskan modal.")  # Translated
    else:
    ...
    with tab12:
        ...
        if not df_inbound_filtered.empty:
            ...
            fig_top_suppliers_qty = px.bar(top_suppliers_qty, x='Nama Supplier', y='Total_Qty_Received',
                                           ...
                                           labels={'Total_Qty_Received': 'Total Kuantitas Diterima (Unit)'},
                                           # Translated
                                           color='Nama Supplier',
                                           template='plotly_white')
    ...
    with tab13:
        ...
        st.markdown(
            "Atur ambang batas untuk metrik kinerja utama. Anda akan melihat peringatan di sini jika metrik berada di bawah ambang batas yang ditentukan.")  # Translated
        ...
        if not df_sales_filtered.empty:
            ...
            with col_status3:
                display_kpi_card("Margin Laba Kotor Saat Ini", f"{current_profit_margin:,.2f}%",
                                 "#FF9800")  # Translated
            ...
            if current_nett_sales < min_sales_threshold:
                st.error(
                    f"🚨 Peringatan: Penjualan Bersih saat ini (Rp {current_nett_sales:,.2f}) berada di bawah ambang batas minimum yang ditetapkan (Rp {min_sales_threshold:,.2f}).")  # Translated
            else:
                st.success(
                    f"✅ Penjualan Bersih saat ini (Rp {current_nett_sales:,.2f}) memenuhi ambang batas.")  # Translated

            if current_gross_profit < min_profit_threshold:
                st.error(
                    f"🚨 Peringatan: Laba Kotor saat ini (Rp {current_gross_profit:,.2f}) berada di bawah ambang batas minimum yang ditetapkan (Rp {min_profit_threshold:,.2f}).")  # Translated
            else:
                st.success(
                    f"✅ Laba Kotor saat ini (Rp {current_gross_profit:,.2f}) memenuhi ambang batas.")  # Translated

            if current_profit_margin < min_profit_margin_threshold:
                st.error(
                    f"🚨 Peringatan: Margin Laba Kotor saat ini ({current_profit_margin:,.2f}%) berada di bawah ambang batas minimum yang ditetapkan ({min_profit_margin_threshold:,.2f}%).")  # Translated
            else:
                st.success(
                    f"✅ Margin Laba Kotor saat ini ({current_profit_margin:,.2f}%) memenuhi ambang batas.")  # Translated
    ...
    with tab14:  # New tab for What-If Analysis
        ...
        st.markdown(
            "Simulasikan dampak perubahan harga atau kuantitas terjual pada penjualan dan laba Anda.")  # Translated
        ...
        if scenario_scope == "Kategori Tertentu":  # Translated
            ...
            if not all_categories_for_whatif:
                st.warning(
                    "Tidak ada kategori yang tersedia untuk simulasi. Unggah data penjualan terlebih dahulu.")  # Translated
                st.stop()
            ...
            if not all_product_names_for_whatif:
                st.warning(
                    "Tidak ada produk yang tersedia untuk simulasi. Unggah data penjualan terlebih dahulu.")  # Translated
                st.stop()
        ...
        if not df_whatif_base.empty:
            ...
            target_rows_mask = pd.Series([True] * len(df_whatif_simulated),
                                         index=df_whatif_simulated.index)  # Default to all
            ...
            with col_whatif_res2:
                display_kpi_card("Penjualan Bersih Hipotetis", f"Rp {hypothetical_total_sales:,.2f}",
                                 "#FF9800")  # Translated
                display_kpi_card("Laba Kotor Hipotetis", f"Rp {hypothetical_gross_profit:,.2f}",
                                 "#673AB7")  # Translated
            ...
            comparison_data_melted = comparison_data.melt(id_vars='Metrik', var_name='Tipe Data',
                                                          value_name='Nilai')  # Translated
            ...
            fig_whatif_comparison = px.bar(comparison_data_melted, x='Metrik', y='Nilai', color='Tipe Data',
                                           ...
                                           labels={'Nilai': 'Jumlah (Rp)', 'Metrik': 'Metrik',
                                                   'Tipe Data': 'Tipe Data'},  # Translated
                                           template='plotly_white')
            ...
            st.info(
                "Tidak ada data penjualan yang tersedia untuk melakukan analisis 'Bagaimana Jika' dalam filter yang dipilih atau untuk item yang dipilih.")  # Translated

    with tab15:  # New tab for Correlation Analysis
        st.subheader("Analisis Korelasi Penjualan vs. Laba Kotor")  # Translated
        st.markdown(
            "Pahami hubungan antara penjualan bersih dan laba kotor pada berbagai tingkat agregasi.")  # Translated
        ...
        if not df_sales_filtered.empty:
            ...
            if correlation_level == "Per Transaksi":  # Translated
                ...
                if 'No Transaksi' not in df_correlation.columns:
                    st.warning(
                        "Kolom 'No Transaksi' tidak ditemukan di data penjualan. Analisis korelasi 'Per Transaksi' tidak dapat dilakukan.")
            ...
            if not df_correlation_agg.empty:
                ...
                correlation_coefficient = df_correlation_agg['Total_Nett_Sales'].corr(
                    df_correlation_agg['Total_Gross_Profit'])
                st.info(
                    f"Koefisien Korelasi Pearson antara Penjualan Bersih dan Laba Kotor{title_suffix}: **{correlation_coefficient:,.2f}**")  # Translated
                ...
                fig_correlation = px.scatter(df_correlation_agg,
                                             ...
                                             title=f'Korelasi Penjualan Bersih vs. Laba Kotor{title_suffix}',
                                             # Translated
                                             labels={'Total_Nett_Sales': x_label, 'Total_Gross_Profit': y_label},
                                             hover_name=group_by_cols[0] if group_by_cols else 'No Transaksi',
                                             # Show item name on hover
                                             template='plotly_white')
                st.plotly_chart(fig_correlation, use_container_width=True)
            else:
                st.info(
                    f"Tidak ada data yang cukup untuk analisis korelasi {correlation_level} dalam filter yang dipilih.")  # Translated
        else:
    ...
    with tab16:  # New tab for Product Price Trend Analysis
        ...
        if not df_sales_filtered.empty and 'Nama Barang' in df_sales_filtered.columns and 'Harga' in df_sales_filtered.columns:
            all_products_for_price_trend = ['Pilih Produk'] + list(
                df_sales_filtered['Nama Barang'].unique())  # Translated
            ...
            if selected_product_for_price_trend != 'Pilih Produk':  # Translated
                df_product_price_trend = df_sales_filtered[
                    df_sales_filtered['Nama Barang'] == selected_product_for_price_trend].copy()
                ...
                if not df_product_price_trend.empty:
                    ...
                    fig_price_trend = px.line(daily_avg_price, x='Tanggal', y='Harga',
                                              title=f'Tren Harga untuk {selected_product_for_price_trend}',
                                              # Translated
                    ...
                    with col_price_summary1:
                        display_kpi_card("Harga Minimum", f"Rp {daily_avg_price['Harga'].min():,.2f}",
                                         "#4CAF50")  # Translated
                    with col_price_summary2:
                        display_kpi_card("Harga Maksimum", f"Rp {daily_avg_price['Harga'].max():,.2f}",
                                         "#2196F3")  # Translated
                    with col_price_summary3:
                        display_kpi_card("Harga Rata-rata", f"Rp {daily_avg_price['Harga'].mean():,.2f}",
                                         "#FF9800")  # Translated
                    ...
                    st.info(
                        f"Tidak ada data harga yang tersedia untuk produk '{selected_product_for_price_trend}' dalam filter saat ini.")  # Translated
            else:
                st.info("Silakan pilih produk untuk melihat tren harganya.")  # Translated
        else:
            st.info(
                "Kolom 'Nama Barang' atau 'Harga' tidak ditemukan di Data Penjualan, atau data penjualan kosong. Analisis tren harga produk tidak tersedia.")  # Translated
    ...
    if 'current_user_id' not in st.session_state or not st.session_state['current_user_id']:
        st.info(
            "Silakan masukkan ID Pengguna Anda di sidebar dan klik 'Login / Muat Data' untuk memulai.")  # Translated
    else:
        # Message for non-admin users who are logged in but have no data
        st.info(
            "Anda masuk sebagai pengguna biasa. Dashboard akan menampilkan data yang terakhir diunggah oleh admin.")  # Translated
        st.markdown("""
